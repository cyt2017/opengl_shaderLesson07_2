...
{
    std::vector<CELL::matrix4> matVects;
    for(int i=0;i<100;i+=20)
    {
        for(int j=0;j<100;j+=20)
        {
            CELL::matrix4 mat;
            mat.translate(CELL::float3(i,0,j));
            matVects.push_back(mat);
        }
    }
    static float angle = 0;
    angle += 0.1f;

    for(int i=0;i<matVects.size();i++)
    {
        CELL::matrix4   matTra = matVects[i];
        CELL::matrix4   matRot;
        matRot.rotateYXZ(angle,angle,angle);

        CELL::matrix4   model   = matTra*matRot;
